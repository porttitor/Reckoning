...
class Parser:
    """A lightweight UBX message parser.
     ...
     Esta classe é projetada para conter um conjunto de classes de mensagem, quando um fluxo é passado via `receive_from`
     método o fluxo é lido até que o PREFIX seja encontrado, o tipo de mensagem corresponde à classe registrada
     e a identificação da mensagem, o comprimento lido, a soma de verificação verificada e, finalmente, a mensagem é desconstruída e retornada
     como uma tupla nomeada.

     Isso é alimentado pelo pacote de estrutura embutido para o trabalho pesado da decodificação da mensagem e há
     sem dependências externas.

     As classes de mensagens podem ser passadas pelo construtor ou pelo método `register_cls`.

     O método `receive_from` não se preocupa com o tipo subjacente do fluxo fornecido, contanto que
     suporta um método `read` e retorna bytes. Cabe à implementação determinar como obter o
     Pacotes UBX do dispositivo. A maneira típica de fazer isso seria um pacote serial como o pyserial, consulte o
     arquivo de exemplos.
